...
def draw_hangul(image, text, position, font_size, color):
...
warning_sounds = [
    "C:/Users/Pang Choi/Desktop/music/sit_up.mp3"
]
...
def draw_lines(image, points, pairs, color, width, height, threshold=0.1):
    if points[pairs[0]][2] > threshold and points[pairs[1]][2] > threshold:
        cv2.line(image,
                 (int(points[pairs[0]][0] * width),
 int(points[pairs[0]][1] * height)),
                 (int(points[pairs[1]][0] * width),
                  int(points[pairs[1]][1] * height)),
                 color, 3, cv2.LINE_AA)
...
def compare_pose(image, angle_point, angle_user, angle_target):
    ...
    if angle_user[4] > (angle_target[4] + 30) and angle_user[4] < (angle_target[4] + 40):
    ...
    if angle_user[5] > (angle_target[5] + 30) and angle_user[5] < (angle_target[5] + 40):
        warning_sound_index = 0  # 경고음 인덱스를 설정합니다. (배열의 0번째 요소를 사용하려면 0으로 설정)
        sound_path = warning_sounds[warning_sound_index]  # 경고음 경로를 불러옵니다.
...
def gen_frames():
    ...
    path = "C:/Users/Pang Choi//Desktop/image/situp.jpg"
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while True:
            ...
            try:
                ...
                if (p_score >= a_score):
                    cv2.putText(image, str(int((1 - a_score)*100)), (100, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255], 2, cv2.LINE_AA)

                else:
                    cv2.putText(image, str(int((1 - p_score)*100)), (100, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255], 2, cv2.LINE_AA)
                
                if(p_score > a_score): score = (1 - a_score) * 100
                else: score = (1 - p_score) * 100
                
                if(score > 75 and stage == 0 ):
                ...
                # 스쿼트 상태 분석 및 카운트
                if score < 40 and exercise_state == 0:
                    exercise_state = 1  # 앉은 상태로 변경

                if score > 75 and exercise_state == 1 and stage == 0:
                    exercise_count += 1  # 스쿼트 완료 후 카운트 증가
                    exercise_state = 0   # 상태 초기화
                
                cv2.putText(image, str(int(exercise_count)), (120, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 2, cv2.LINE_AA)                          
