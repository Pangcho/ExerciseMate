...
warning_sounds = [
    ...
    "C:/Users/Pang Choi/Desktop/music/down_hip.mp3",
    "C:/Users/Pang Choi/Desktop/music/up_hip.mp3",
    "C:/Users/Pang Choi/Desktop/music/extend_knee.mp3"
]
...
def compare_pose(image, angle_point, angle_user, angle_target):
    ...
    if angle_user[4] < (angle_target[4] - 15):
        warning_sound_index = 2  # 경고음 인덱스를 설정합니다. (배열의 0번째 요소를 사용하려면 0으로 설정)
        sound_path = warning_sounds[warning_sound_index]  # 경고음 경로를 불러옵니다.
    ...
    if angle_user[4] > (angle_target[4] + 15) and angle_user[4] < (angle_target[4] + 80) :
        warning_sound_index = 3  # 경고음 인덱스를 설정합니다. (배열의 0번째 요소를 사용하려면 0으로 설정)
        sound_path = warning_sounds[warning_sound_index]  # 경고음 경로를 불러옵니다.
    ...
    if angle_user[6] < (angle_target[6] - 15):
        warning_sound_index = 4  # 경고음 인덱스를 설정합니다. (배열의 0번째 요소를 사용하려면 0으로 설정)
        sound_path = warning_sounds[warning_sound_index]  # 경고음 경로를 불러옵니다.
...
def gen_frames():
    ...
    path = "C:/Users/Pang Choi//Desktop/image/pushup.jpg"
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while True:
            ...
            try:
                ...
                if score > 75 and exercise_state == 1 :
                    exercise_count += 1  # 스쿼트 완료 후 카운트 증가
                    exercise_state = 0   # 상태 초기화
                
                cv2.putText(image, str(int(exercise_count)), (200, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 2, cv2.LINE_AA)                          
